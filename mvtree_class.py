...
class MVTreeFeatureExtractor(_MVTreeExtractor):
    ...
    def transform(self, data, y=None):
      ...
      if len(self.time_features) > 0:
        input_params = {"date_range":pd.DatetimeIndex(data[self.ts_date]),
        ...
        if (len(self.window_functions) > 0):
          #  lag_kwargs = [{"lag":lag} for lag in lags]  
            ...
            grouped =  data.loc[:, self.ts_uid+[self.ts_values]].groupby(self.ts_uid)[self.ts_values]
        ...
        #collect_length = [(i["window_shift"]+i["window_size"]) for i in input_kwargs ]
        #collect_max = max(collect_length)
        all_features = pd.concat(all_features_list, axis=1)
        #all_features = all_features[(collect_max-1):]
        #data = data[(collect_max-1):]
        # merging all features
        data.drop(self.exclude_features, axis=1,inplace=True)
